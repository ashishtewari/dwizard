...
public class HandleException {
	...
	public Reply accessTokenException(int validateResponse) {
		...
		if(validateResponse == 0){
			...
			errorLog.warn("Invalid userName specified");
			return exception.getException("Invalid userName specified",null);
		}
			...
			exception = new HandleException(Response.Status.UNAUTHORIZED.getStatusCode(),Response.Status.UNAUTHORIZED.getReasonPhrase());
			errorLog.warn("You are not an active user");
			return exception.getException("You are not an active user",null);
		}
		...
		else if(validateResponse == -5){
			exception = new HandleException(Response.Status.BAD_REQUEST.getStatusCode(),Response.Status.BAD_REQUEST.getReasonPhrase());
			errorLog.warn("You are not authorized to access this function");
			return exception.getException("You are not authorized to access this function",null);
		}
		else {
			exception = new HandleException(Response.Status.BAD_REQUEST.getStatusCode(),Response.Status.BAD_REQUEST.getReasonPhrase());
